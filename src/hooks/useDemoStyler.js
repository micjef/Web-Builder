...
const useDemoStyler = ({ id }) => {
  const parentSelector = (selectors) => selectors.selectParent(id);
  const parent = useCollector({
    selector: parentSelector,
  });
  function styles() {
    const flexItem = parent.node && parent.node.props.style.display === "flex";
    const positioned =
      !parent.node || (parent.node.type !== "Grid" && !flexItem);
