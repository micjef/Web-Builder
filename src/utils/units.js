...
export const convertToPx = (measure, options = {}) => {
  const {
    target = document,
  ...
  const appendingToElement = targetAsContainer ? target : target.parentElement;
  ...
  Object.keys(withProperties).forEach(
    (property) => (element.style[property] = withProperties[property])
  );
  ...
  const offsetParentRect =
    calculatedOffsetParentRect || defaultOffsetParentRect;
  ...
  const rect = {
    top: elementRect.top + offsetParentScroll.top - offsetParentRect.top,
    left: elementRect.left + offsetParentScroll.left - offsetParentRect.left,
