...
const Anchor = ({
  ...
  useEffect(() => {
    ...
    const getDirections = () => {
      const directionX =
        expandsRight === true ? 1 : expandsRight === false ? -1 : 0;
      const directionY =
        expandsDown === true ? 1 : expandsDown === false ? -1 : 0;
      return {
        x: directionX,
        y: directionY,
      };
    ...
    const handleResize = (event) => {
      if (!resizing.current) return;
      const pageX = isNumber(event.pageX)
        ? event.pageX
        : event.touches[0].pageX;
      const pageY = isNumber(event.pageY)
        ? event.pageY
    ...
    cancelClickOnEnd &&
      document.addEventListener("click", handleClickCancel, { capture: true });
    ...
    return () => {
      ...
      cancelClickOnEnd &&
        document.removeEventListener("click", handleClickCancel, {
          capture: true,
        });
  ...
  const startResize = useCallback((event) => {
    const startX = isNumber(event.pageX) ? event.pageX : event.touches[0].pageX;
    const startY = isNumber(event.pageY) ? event.pageY : event.touches[0].pageY;
    setResize({
      startX: startX,
      startY: startY,
    });
  ...
  const classAll = clsx(className, classes.anchor);
  return (
    <button
      tabIndex={-1}
