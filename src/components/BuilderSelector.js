...
const BuilderSelector = () => {
  ...
  const triggerListIndexClear = actions.unrecorded.triggerListIndexClear;
  const selector = (selectors) => selectors.selectMultipleByIndex("selected");
  const collected = useMultiCollector({
    selector: selector,
  });
  const hasSelected = collected.listNodes().length > 0;
  useEffect(() => {
